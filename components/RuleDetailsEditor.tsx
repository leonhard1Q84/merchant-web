      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl my-8" onClick={e => e.stopPropagation()}>
        ...
        <main className="p-6 space-y-6">
          ...
          />
           ...
           <div>
            <label className="block text-sm font-medium text-gray-700">Customer Origin</label>
            <select 
                className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                value={editedRule.conditions.customerOrigin?.condition ?? 'none'}
                onChange={e => {
                    const val = e.target.value;
                    if (val === 'none') {
                        const { customerOrigin, ...restConditions } = editedRule.conditions;
                        setEditedRule(prev => ({ ...prev, conditions: restConditions }));
                    } else {
                        handleConditionChange({ customerOrigin: { condition: val as CustomerOriginCondition, countries: editedRule.conditions.customerOrigin?.countries || [] }});
                    }
                }}
            >
                <option value="none">Not specified (applies to all)</option>
                <option value={CustomerOriginCondition.Include}>Include specific countries</option>
                <option value={CustomerOriginCondition.Exclude}>Exclude specific countries</option>
            </select>
            {editedRule.conditions.customerOrigin && (
                <div className="mt-2">
                    <label htmlFor="countries" className="text-sm text-gray-600">Countries (comma-separated ISO codes, e.g., US,JP,CN)</label>
                    <input
                        id="countries"
                        type="text"
                        value={editedRule.conditions.customerOrigin.countries.join(',')}
                        onChange={e => {
                            const countries = e.target.value.split(',').map(c => c.trim().toUpperCase()).filter(Boolean);
                            handleConditionChange({ customerOrigin: { ...editedRule.conditions.customerOrigin!, countries }})
                        }}
                        className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                    />
                </div>
